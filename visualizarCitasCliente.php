...
$consulta = "SELECT citas.IDCita, citas.mascota, citas.razonCita, citas.fecha, citas.hora,
    ...
    FROM citas
    INNER JOIN cliente ON cliente.IDCliente = citas.IDCliente WHERE cliente.IDCliente = 1 ORDER BY citas.fecha DESC";


$ejecutarConsulta = mysqli_query($enlace, $consulta);

if(!$ejecutarConsulta){
    echo"<p>Error en mostrar las citas</p>";
}else{
    $num = 0;
    while($recuperar = mysqli_fetch_array($ejecutarConsulta)){
        ...
        if(mysqli_num_rows($ejecutarConsulta) > 0){
            ...
            echo'<div class="appointment" style="border: 1px solid #ddd;text-align: center;">
                ...
                <button style="border: 2px solid #d99595; color: #d99595;  background-color: #f9f9f9;" onclick="borrar('.$recuperar['IDCita'].')">Borrar</button><br>
            </div>';
